...
class ControlLoop:
    ...
    def __init__(self, command_queue: asyncio.Queue, config: TelegripConfig, control_commands_queue: Optional[queue.Queue] = None):
        ...
        # 以下为2025-8-14新增  用于piper机械臂控制
        self.piper = None
        self.piper_connected = False

        # 如果config里启用了机器人
        if self.config.enable_robot:
            try:
                self.piper = C_PiperInterface_V2("can_arm2")    # 要先运行SDK的shell脚本激活对应的can端口，比如这里的can_arm2
                logger.info("Piper机械臂实例创建成功")
            except Exception as e:
                logger.error(f"Piper机械臂实例创建失败: {e}")
    ...
    async def start(self):
        ...
        if self.piper:
            ...
            logger.info("Piper机械臂已连接")
            self.piper_connected = True
        # Main control loop
    ...
    def _initialize_arm_states(self):
        ...
        if self.robot_interface:
            ...
            left_pos = self.robot_interface.get_current_end_effector_position("left")# x y z位置坐标
    ...
    async def _execute_goal(self, goal: ControlGoal):
        ...
        if (goal.metadata and goal.metadata.get("reset_target_to_current", False)):
            if self.robot_interface and arm_state.mode == ControlMode.POSITION_CONTROL:
                ...
                current_position = self.robot_interface.get_current_end_effector_position(goal.arm)# x y z 位移
                current_angles = self.robot_interface.get_arm_angles(goal.arm)# 6个关节角
