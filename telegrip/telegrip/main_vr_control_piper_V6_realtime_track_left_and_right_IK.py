"""
这版代码实现了在V5代码的基础上将姿态部分从直接末端控制改为IK解算+关节角控制 但需要优化控制卡顿的问题
"""
import asyncio
import time
import logging
...
class SimpleVRSystem:
    def __init__(self, config: TelegripConfig):
        ...
        self.control_interval = 0.005# 控制频率：250 Hz
    ...
    def quaternion_to_rotation_matrix(self, quaternion: np.ndarray) -> np.ndarray:
        ...
        # time1 = time.time()
        # print(f"旋转矩阵转换时间: {(time1 - time0)*1000:.2f} ms")
    ...
    async def _init_robot_arms(self):
        ...
        try:
            ...
            # 初始化逆解器
            urdf_path = '/home/robotgym/qijia-teleopvr/telegrip/URDF/piper/piper_description.urdf'
    ...
    async def _display_loop(self):
        ...
        while self.is_running:
            ...
            # time2 = time.time()
            # print(f"手柄姿态更新时间: {time2 - time1:.4f}s")
            # 计算新的手柄位姿 --> 计算机械臂目标位姿 --> IK --> 控制机械臂
            # time3 = time.time()
            await self._process_realtime_control()
            # time4 = time.time()
            # print(f"处理实时控制机械臂时间: {time4 - time3:.4f}s")
            # await asyncio.sleep(0.001)  # 主循环
            time.sleep(0.001)  # 主循环
            # time5 = time.time()
            # print(f"主循环耗时(ms):{(time5 - time4) * 1000:.2f}")
                ...
                if controller.accumulated_rotation_quat is not None:
                    try:
                        ...
                        if current_time - self.last_control_time[arm] >= self.control_interval:
                            ...
                            T1[:3, :3] = T1_rot
                            T1[:3, 3] = T1_pos
                            # time_T1_end = time.time()
                            # print(f"构造T1耗时(ms):{(time_T1_end - time_T1_start)*1000} ms")
                            ...
                            if self._cached_transform[arm] is not None and self.m_offset[arm] is not None:
                                ...
                                # print(f"计算A1 耗时: {(A1_end_time - A1_start_time) * 1000:.2f}ms")
                            else:
                            ...
                            # print(f"{arm}IK完成，耗时: {(ik_end_time - ik_start_time)*1000} ms")
                            ...
                            if joint_angles_deg is not None:
                                ...
                                # print(f"滤波函数耗时(ms): {(filter_end_time - filter_start_time) * 1000}")
                                self.last_control_time[arm] = current_time
    ...
    def _move_arm_to_joints(self, arm: str, joint_angles_deg: list, gripper_position: float):
        ...
        try:
            ...
            piper.GripperCtrl(abs(gripper_pos), 1000, 0x01, 0)
            # t2 = time.time()
            # print(f"piperAPI调用耗时2(ms):", {(t2 - t1)*1000})
        except Exception as e:
