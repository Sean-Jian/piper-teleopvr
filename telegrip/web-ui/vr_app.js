...
AFRAME.registerComponent('controller-updater', {
  init: function () {
    ...
    this.leftHand.addEventListener('xbuttondown', (evt) => {
        ...
        // 只有在未按住grip时才发送X键消息（与右手柄A键保持一致）
        if (!this.leftGripDown) {
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                const xButtonMessage = {
                    hand: 'left',
                    xButtonPressed: true,
                    action: 'return_to_home'
                };
                this.websocket.send(JSON.stringify(xButtonMessage));
                console.log('Sent X button press for left hand (return to home)');
            }
        }
    });

    this.leftHand.addEventListener('xbuttonup', (evt) => {
        console.log('Left X Button Released');
        this.leftXDown = false;
    });
    // right--> A
    this.rightHand.addEventListener('abuttondown', (evt) => {
        ...
        if (!this.rightGripDown) {
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                ...
                console.log('Sent A button press for right hand (return to home)');
            }
        }
    });
